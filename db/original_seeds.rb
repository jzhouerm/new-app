...
csv.first(4).each do |row|
    ...
    g.date = Faker::Time.forward(days: 30,  period: :evening, format: :long)
    g.location_id = Location.all.sample.id
    g.description = row['description']
    g.user_id = User.all.sample.id
    g.interest_id = Interest.first
    g.img_url = row['img_url']
    g.save
end
